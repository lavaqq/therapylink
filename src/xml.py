...
def create(data, sendernr, sendername, receivernr, receivername, start_date, end_date, base_filename):
    log_filename = setup_logging(base_filename)
    ...
    for resident in data:
        ...
        for medication_id, medication_details in resident['medications'].items():
            ...
            if cnk is not None:
                ...
                schedule = medication_details.get('schedule', {})
                if isinstance(schedule, dict):
                    ...
                    logging.error(f"Unexpected schedule format for medication ID {medication_id}. (Resident ID : {resident.get('id', '')})")
            else:
                logging.error(f"Medication ID {medication_id} is missing a CNK value and will not be processed. (Resident ID : {resident.get('id', '')})")
                pass
    ...
    print(f"XML and log files created.")
    return filename

def setup_logging(base_filename, output_dir='output'):
    log_filename = f"{output_dir}/{base_filename}_{datetime.now().strftime('%Y%m%d%H%M%S')}_TH.log"
    logging.basicConfig(filename=log_filename, level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s', datefmt='%Y-%m-%dT%H:%M:%S')
    return log_filename